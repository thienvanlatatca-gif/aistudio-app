...
export default function App() {
  ...
  // Two separate inputs: one for folder, one for files
  ...
  const openFolder = (id: number) => { pickId.current = id; folderRef.current?.click(); };
  const openFiles  = (id: number) => { pickId.current = id; filesRef.current?.click(); };

  const handlePicked = (rawFiles: FileList | null, mode: "folder" | "files") => {
    const id = pickId.current; if (id === null) return;
    const files = Array.from(rawFiles ?? []).filter(f => isVideo(f.name));
    if (!files.length) return;
    const vf: VFile[] = files.map(f => ({ name: f.name, file: f, size: f.size }));
    const folderName = mode === "folder"
      ? (((files[0] as any).webkitRelativePath ?? "").split("/")[0] || "Thư mục")
      : `${files.length} file`;
    setRows(r => r.map(row => {
  ...
  return (
    <div style={S.root}>
      ...
      <input ref={folderRef} type="file" multiple style={{display:"none"}}
        onChange={e => { handlePicked(e.target.files, "folder"); e.target.value=""; }}
        {...{webkitdirectory:"", directory:""} as any} />
      {/* Individual files picker */}
      <input ref={filesRef} type="file" multiple accept="video/*" style={{display:"none"}}
        onChange={e => { handlePicked(e.target.files, "files"); e.target.value=""; }} />
        ...
        <div style={S.hdr}>
          <div>
            ...
            <div style={S.sub}>{_ready
              ? "// FFmpeg.wasm · ghép trong trình duyệt · tải video xuống"
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:8}}>
          {rows.map((row,i) => (
            <RowCard key={row.id} row={row} idx={i+1} disabled={busy}
              onFolder={()=>openFolder(row.id)}
              onFiles={()=>openFiles(row.id)}
              onDel={()=>deleteRow(row.id)}
              onRand={()=>randRow(row.id)}
              onQtyD={d=>changeQty(row.id,d)}
              onQtyInp={v=>inputQty(row.id,v)} />
          ))}
        </div>
...
function RowCard({row,idx,disabled,onFolder,onFiles,onDel,onRand,onQtyD,onQtyInp}:{
  row:Row;idx:number;disabled?:boolean;
  onFolder:()=>void;onFiles:()=>void;onDel:()=>void;onRand:()=>void;
    ...
    <div style={S.card}>
      <div style={S.cardTop}>
        ...
        <span style={{...S.cardName,...(has?S.cardOn:{})}}>
          {has?`📁 ${row.folderName}  (${row.files.length} video)`:"Chưa chọn video"}
        </span>
      ...
      <div style={S.ctrl}>
        {/* Folder button */}
        <button style={S.pickBtn} onClick={onFolder} disabled={disabled} title="Chọn cả thư mục (ổ cứng trong)">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
          Chọn thư mục
        </button>
        {/* Files button */}
        <button style={{...S.pickBtn, color:"#4da6ff", borderColor:"#1a2a3a"}} onClick={onFiles} disabled={disabled} title="Chọn từng file (ổ cứng rời / USB)">
